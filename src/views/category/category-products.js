...
class CategoryProducts extends Component {
      ...
      <Animatable.View animation={'fadeInDown'} delay={500} duration={300}>
        <View style={filterStyles.categoryFilters}>
          ...
          <View style={filterStyles.listLayoutButtonView}>
            <TouchableOpacity
              activeOpacity={layout == 'list' ? 1 : 0}
              style={filterStyles.listLayoutButton}
              onPress={() => {
                this._changeLayout('list');
              }}>
              <Image
                source={layout == 'list' ? ic_list_view_active : ic_list_view}
                resizeMode={'contain'}
                style={{width: 16, height: 15}}
              />
            </TouchableOpacity>
            <View style={filterStyles.lineOfLayoutButtons} />
            <TouchableOpacity
              activeOpacity={layout == 'grid' ? 1 : 0}
              style={filterStyles.gridLayoutButton}
              onPress={() => {
                this._changeLayout('grid');
              }}>
              <Image
                source={layout == 'grid' ? ic_grid_view_active : ic_grid_view}
                resizeMode={'contain'}
                style={{width: 16, height: 16.41}}
              />
            </TouchableOpacity>
          </View>
          <View style={filterStyles.lineOfLayoutButtons2} />
  ...
  _changeLayout(type) {
    ...
    const _itemWidth =
      _columns === 1 ? window_width - padd : window_width / 2 - padd / 2; //((width - (padd * _columns)) / _columns) - (padd / _columns)
    let _imageWidth =
      _columns === 1 ? _itemWidth - padd * 2 : _itemWidth - padd;
  ...
  renderFooter() {
    return (
      <View style={styles.footer}>
        {this.state.fetching_from_server ? <Loading /> : null}
      </View>
    );
  }
  ...
  _renderItem(item) {
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() =>
          navigation.navigate('ProductScreenInCategoryScreen', {product: item})
        }
        underlayColor="white">
        <Animated.View
          style={[
        ...
        {/* eslint-disable-next-line react-native/no-inline-styles */}
        <Animatable.View
          animation={key > 2 ? (layout == 'list' ? 'fadeInUp' : 'fadeIn') : ''}
          duration={700}
          style={[
            layout == 'list'
              ? {
          ...
          <Text
            numberOfLines={layout == 'list' ? 5 : 1}
            style={productStyles.name}>
            {item.name}
          </Text>
          {/* eslint-disable-next-line react-native/no-inline-styles */}
          <View
            ...
            item.price + '' !== item.regular_price + '' ? (
              <View style={productStyles.prices}>
                <Text style={productStyles.price_discounted}>
                  ${item.regular_price}
                </Text>
      ...
      <View style={styles.container}>
        <View style={{marginTop: 15}}>
          <Item style={styles.searchBar}>
            ...
            <Input
              editable={false}
              placeholder={navigation.state.params.item.name.toUpperCase()}
        ...
        {this._renderHeader()}
        <View style={styles.tabContent}>{this._renderProducts()}</View>
      </View>
    );
  }
}
