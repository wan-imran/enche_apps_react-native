...
export function register(first_name, last_name, email, user_name, password) {
  const promise = new Promise(function(resolve, reject) {
      ...
      fsManager
        ...
        .then(() => {
          ...
          fetch(customerURL)
            ...
            .then(json => {
              fsManager
                .saveCustomer(JSON.stringify(json))
                .then(() => {
                  resolve('success');
                })
                .catch(err => {
    ...
    function regiterUser(nonce) {
      registerURL += `nonce=${nonce}&username=${user_name}&display_name=${first_name +
        ' ' +
        last_name}&email=${email}&user_pass=${password}`;
      fetch(registerURL)
        .then(res => res.json())
        .then(json => {
          if (json.status === 'ok') {
